...
for (i in seq(3)) {
  ...
  while (TRUE) {
    ...
    if (t == 0) {
      ...
      # remove those that have already sit down from the queue
      updated_Q = updated_Q[updated_Q != which.min(sitdown_time_list)]
      # after updating each person's info (the three lists), we jump to the next t when something happens
      t = min(sitdown_time_list)
      # reset the sitdown_time of those who have finished sitting down back to Inf so we won't choose them later
    ...
    # general (happens most of the time)
      ...
      # add something that changes the distribution parameter of storing luggage
      # assume when 4/5 boarded, 1/5 will have luggage gate checked
      if (length(which(!standing_list))/length(original_Q) >= 4/5){
        # no longer stores luggage
        lugg_min = 0
        lugg_max = 0
      }
      for (i in 1:length(updated_Q)) {
        ...
        if (standing_list[seat_num]) {
          ...
          if (location_list[seat_num] == ceiling(seat_num/seats_per_row)) {
            if (seat_num%%seats_per_side == 1) {
              ...
              sitdown_time_list = gen_window(sitdown_time_list, seat_num, rates, lugg_min, lugg_max)
            } # end window
